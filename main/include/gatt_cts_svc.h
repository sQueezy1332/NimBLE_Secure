...
static int ble_svc_cts_chr_write(os_mbuf *om, uint16_t min_len, uint16_t max_len, void *dst, uint16_t *len);
...
static const struct ble_gatt_svc_def ble_svc_cts_defs[] = { 
    {       /*** Current Time Service. */
        ...
        .uuid = &CTS_UUID16.u,
        .characteristics = (ble_gatt_chr_def[]) { { /*** Current Time characteristic */
            .uuid = &CHR_UUID16_CURRENT_TIME.u,
            .access_cb = ble_svc_cts_access,
            .flags = BLE_GATT_CHR_F_READ | BLE_GATT_CHR_F_WRITE_AUTHOR |  BLE_GATT_CHR_F_NOTIFY,
	        .val_handle = &ble_svc_cts_curr_time_handle,
	    }, {                                            /*** Local info characteristic */
            .uuid = &CHR_UUID16_LOCAL_TIME_INFO.u,
            .access_cb = ble_svc_cts_access,
            .flags = BLE_GATT_CHR_F_READ | BLE_GATT_CHR_F_WRITE_AUTHOR,
...
void set_cts_unix(time_t now) {
    ...
    gettimeofday(&last_updated, NULL);  /* set the last updated */
...
void fetch_current_time(struct ble_svc_cts_curr_time *ctime) {
    time_t now; timeval tv_now;
    time(&now);
    struct tm *timeinfo = localtime(&now); if(!timeinfo) return ;
    ...
    if(ctime) {
        ...
        ESP_LOGI(CTS, "day_of_week %u", ctime->et_256.d_d_t.day_of_week);
        /* fractions_256 */
        ctime->et_256.fractions_256 = (((uint64_t)tv_now.tv_usec * 256 ) / 1000000L);
...
int ble_svc_cts_chr_write(os_mbuf *om, void *dst, uint16_t max_len, uint16_t min_len = 0, uint16_t *len = NULL) {
    uint16_t om_len = OS_MBUF_PKTLEN(om);
    if (om_len < min_len || om_len > max_len) { return BLE_ATT_ERR_INVALID_ATTR_VALUE_LEN; }
    CHECK_RETURN(ble_hs_mbuf_to_flat(om, dst, max_len, len));
    return 0;
}
...
static int ble_svc_cts_access(uint16_t conn_handle, uint16_t attr_handle, ble_gatt_access_ctxt *ctxt, void *arg) {
    ...
    case BLE_SVC_CTS_CHR_UUID16_CURRENT_TIME:
        ...
        case BLE_GATT_ACCESS_OP_WRITE_CHR: {
            if(ctxt->om->om_len != sizeof(ble_svc_cts_curr_time)) return BLE_ATT_ERR_UNLIKELY;
            const auto& curr_time = *reinterpret_cast<ble_svc_cts_curr_time*>(ctxt->om->om_data);
            //CHECK_RETURN(ble_hs_mbuf_to_flat(ctxt->om, &curr_time, sizeof(curr_time)));
            CHECK_RETURN(ble_svc_cts_curr_time_validate(curr_time));
            CHECK_RETURN(ble_svc_cts_chr_write(ctxt->om, &current_local_time_val, sizeof(current_local_time_val)));
    ...
    case BLE_SVC_CTS_CHR_UUID16_LOCAL_TIME_INFO:
        ...
        case BLE_GATT_ACCESS_OP_READ_CHR: ESP_LOGI(CTS, "get_local_tz");
            ...
            return 0;
        case BLE_GATT_ACCESS_OP_WRITE_CHR: { 
            if(ctxt->om->om_len != sizeof(ble_svc_cts_local_time_info)) return BLE_ATT_ERR_UNLIKELY;
            auto time_info = *reinterpret_cast<ble_svc_cts_local_time_info*>(ctxt->om->om_data);
            //CHECK_RETURN(ble_hs_mbuf_to_flat(ctxt->om, &local_time_info, 2, NULL)); 
            CHECK_RETURN(ble_svc_cts_local_time_info_validate(time_info));
            CHECK_RETURN(ble_svc_cts_chr_write(ctxt->om,  &time_info, sizeof(time_info)));
    ...
    case BLE_SVC_CTS_CHR_UUID16_REF_TIME_INFO:
        ...
        } else { ESP_LOGW(CTS, "REF_TIME_INFO ctxt->op %u", ctxt->op); return BLE_ATT_ERR_UNLIKELY; }
        return 0;
